...
describe('API calling', () => {
  ...
  it('Should make error post request of send()', async () => {
    serviceRequest.method = 'POST';
    mock.post(endpoint, {
      body: '{"statusText":"Please verify the Worldline Device API URL"}',
      status: 405
    });
    try {
      await serviceRequest.send();
    } catch (err) {
      expect(err).toEqual(errPostResponse);
    }
  });
  ...
  it('Should make error get request of send()', async () => {
    ...
    try {
      ...
      expect(err).toEqual(errGetResponse);
    }
  });
